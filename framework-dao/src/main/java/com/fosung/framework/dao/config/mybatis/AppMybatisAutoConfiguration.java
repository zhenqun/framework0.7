package com.fosung.framework.dao.config.mybatis;

import com.fosung.framework.common.config.AppProperties;
import com.fosung.framework.common.json.JsonMapper;
import com.fosung.framework.dao.config.AppDaoProperties;
import com.fosung.framework.dao.config.datasource.AppDataSourceTransactionAutoConfiguration;
import com.fosung.framework.dao.config.mybatis.meta.JPAObjectWrapperFactory;
import com.fosung.framework.dao.config.mybatis.page.support.PageInterceptor;
...
@ConditionalOnProperty(name = AppDaoProperties.PREFIX + ".mybatis.enabled" , matchIfMissing = true , havingValue = "true")
@Import(value = { AppDataSourceTransactionAutoConfiguration.class })
public class AppMybatisAutoConfiguration {
	...
	public SqlSessionFactoryBean sessionFactoryBean(DataSource dataSource) throws IOException{
		log.info("sessionFactoryBean===="+ JsonMapper.toJSONString(appDaoProperties , true));
	...
	protected void initPagePlugin(List<Interceptor> interceptorList){
		...
		properties.put("dialect", appDaoProperties.getMybatis().getDialect( appDaoProperties.getDatasource().getUrl() ) ) ;
		//不进行合理化查询优化
